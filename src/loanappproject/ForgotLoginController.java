...
public class ForgotLoginController implements Initializable {
    ...
    private void submitButton() {
        ...
        if (!mail.matches("^[A-Za-z0-9+_.-]+@gmail\\.com$")) {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("InvalidEmail.fxml"));
                Parent root = loader.load();
        ...
        if (!phone.matches("^(\\+234|234|0)[789][01]\\d{8}$")) {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("InvalidPhone.fxml"));
                Parent root = loader.load();

                Stage stage = new Stage();
                stage.setScene(new Scene(root));
                stage.setTitle("Invalid Phone");
                stage.initModality(Modality.APPLICATION_MODAL);
                stage.showAndWait();
            } catch (Exception e) {
                e.printStackTrace();
